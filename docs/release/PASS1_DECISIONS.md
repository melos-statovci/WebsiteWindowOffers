...
## RC-02 raw Better Auth organization lifecycle policy — implemented

Better Auth 1.6.27 remains authoritative for session, membership, role and last-owner checks. Kornizo adds its canonical account-availability decision only to state-expanding organization mutations. `TRIAL` and `ACTIVE` pass the lifecycle gate; `TRIAL_EXPIRED`, `SUSPENDED`, and a missing `organization_accounts` row (`ACCOUNT_NOT_READY`) fail before the first write. Cleanup/self-protective mutations are not lifecycle-blocked.

| Operation / route | Better Auth authority | Lifecycle class | Kornizo control | Trusted provisioning overlap | Write-order evidence / final behavior |
|---|---|---|---|---|---|
| Create organization — `POST /organization/create` | Session users require `allowUserToCreateOrganization`; the no-session `userId` form is server-only | PUBLIC ALWAYS BLOCK | Existing `beforeCreateOrganization` validates server-trusted identity | Yes: initial provisioning uses the same API operation without an HTTP request and supplies `userId` inside trusted context | Browser creation is denied before insert; trusted organization/owner creation remains unchanged |
| Delete organization — `POST /organization/delete` | `disableOrganizationDeletion` is checked before session and `organization:delete` permission | ALWAYS BLOCK | `disableOrganizationDeletion:true` | None | Refusal precedes active-session clearing or deletion; full organization/member/invitation snapshot stays unchanged |
| Update organization — `POST /organization/update` | Membership plus `organization:update`; slug conflict checked | LIFECYCLE-GATED | `beforeUpdateOrganization` checks the resolved member organization | No trusted provisioning/repair use | Hook precedes `updateOrganization`; unavailable tenants retain all fields |
| Create invitation — `POST /organization/invite-member` | Membership plus `invitation:create`; owner-only owner-role rule | LIFECYCLE-GATED | Path-specific Better Auth `hooks.before` gate | None | Gate precedes provider routing, including new, resend-expiry and cancel-on-reinvite branches; provider permissions still decide after a pass |
| Accept invitation — `POST /organization/accept-invitation` | Matching recipient, verification policy, pending/unexpired invitation and membership limit | LIFECYCLE-GATED | `beforeAcceptInvitation` checks `invitation.organizationId` | None | Hook precedes accepted status, member insert and active-org change; denied invitation stays pending with no member |
| Reject invitation — `POST /organization/reject-invitation` | Matching recipient plus provider verification policy | USER-PROTECTIVE | No lifecycle hook | None | Provider-authorized rejection intentionally changes only invitation status and remains available |
| Cancel invitation — `POST /organization/cancel-invitation` | Membership plus `invitation:cancel` | USER-PROTECTIVE / CLEANUP | No lifecycle hook | None | Provider-authorized cancellation intentionally reduces pending access and remains available |
| Remove member — `POST /organization/remove-member` | Actor membership, owner/last-owner rules and `member:delete` | LIFECYCLE-GATED ADMIN ACTION | `beforeRemoveMember` checks the resolved target organization | No; trusted missing-owner repair uses server-only `addMember` | Hook precedes member delete and active-session clearing; unavailable tenant membership stays unchanged |
| Update member role — `POST /organization/update-member-role` | Actor membership, valid role, owner invariants and `member:update` | LIFECYCLE-GATED | `beforeUpdateMemberRole` checks the resolved target organization | None | Hook precedes `updateMember`; unavailable tenant role stays unchanged |
| Leave organization — `POST /organization/leave` | Signed-in membership and provider last-owner rule | USER-PROTECTIVE | No lifecycle hook; this route does not invoke `beforeRemoveMember` | None | Provider may delete the caller's membership/clear active org even when unavailable, but still refuses the last owner |
| Set active organization — `POST /organization/set-active` | Signed-in membership; may also clear active org | PROVIDER-PERMISSION | No lifecycle hook | Provisioning test helpers may activate after trusted provisioning; core provisioning does not depend on it | Session-only mutation remains available; canonical tenant context still returns `TRIAL_EXPIRED`, `SUSPENDED`, or `ACCOUNT_NOT_READY` and grants no business access |

The remaining installed POST endpoints, `/organization/check-slug` and `/organization/has-permission`, are read-only probes. `addMember` is a server-only endpoint and is retained exclusively for trusted owner-membership repair. Team mutations are not registered because teams are disabled; dynamic-role mutations are not registered because dynamic access control is disabled. Thus no additional browser/session-reachable organization mutation exists in this configuration.

The invitation route needs the narrow path-specific before gate because 1.6.27 handles `resend:true` (and optional cancel-on-reinvite) before `beforeCreateInvitation`. Direct HTTP regression freezes the pending invitation expiry/status and proves zero partial write. All other lifecycle hooks listed above execute after provider target/permission resolution but before their first adapter write. No schema, transaction, provider authority, or trusted provisioning contract changed.

RC-02, RC-06/07, RC-11, and RC-12 received explicit approval and are implemented. RC-05 remains open.
