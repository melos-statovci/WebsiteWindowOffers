...
export default async function PlatformDashboardPage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="font-heading text-2xl font-semibold text-white">Përmbledhje</h1>
      ...
      <PanelCard
        ...
        action={
          <Link href="/platform/organizations" className="text-sm text-indigo-300 hover:text-indigo-200">
            Shiko organizatat →
          </Link>
        }
      >
        <div className="grid grid-cols-3 gap-4">
          {PLAN_TIERS.map((tier) => (
            <div key={tier} className="rounded-lg border border-slate-800 bg-slate-950/40 p-4">
              <PlanBadge plan={tier} />
              <div className="mt-2 font-heading text-2xl font-semibold text-white">{o.planDistribution[tier]}</div>
            </div>
          ))}
        </div>
      </PanelCard>
