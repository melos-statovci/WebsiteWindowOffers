...
import {
  ...
  type OrgCommercialAccessFilter,
  type OrgSort,
...
import { CommercialAccessBadge, PanelCard, PlanBadge, StatusBadge } from "../ui";
...
export default async function OrganizationsListPage({
  ...
  searchParams: Promise<{ q?: string; status?: string; plan?: string; access?: string; sort?: string }>;
}) {
  ...
  const orgs = await listPlatformOrganizations({ q, status, plan, commercialAccess: access, sort });
  ...
  return (
    <div className="space-y-6">
      ...
      <form method="get" className="flex flex-wrap items-end gap-3 rounded-2xl border border-slate-200 bg-slate-100 p-4">
        ...
        <div>
          <label className="mb-1 block text-xs font-medium text-slate-400">Qasja</label>
        ...
        {(q || status !== "all" || access !== "all" || plan !== "all" || sort !== "created_desc") && (
            ...
            <table className="w-full text-left text-sm">
              <thead>
                <tr className="border-b border-slate-200 text-xs uppercase tracking-wide text-slate-400">
                  ...
                  <th className="py-2 pr-4 font-medium">Qasja</th>
                  ...
                  <tr key={o.id} className="border-b border-slate-200/70 last:border-0 hover:bg-slate-50">
                    ...
                    <td className="py-3 pr-4">
                      <div className="space-y-1">
                        <CommercialAccessBadge access={o.effectiveCommercialAccess} />
                        {o.effectiveCommercialAccess === "trial" && (
                          <div className="text-xs text-slate-400">{o.trialDaysRemaining} ditë</div>
                        )}
                      </div>
                    </td>
