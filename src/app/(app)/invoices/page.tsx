...
import { Plus, Search, Receipt, DollarSign, Check, TriangleAlert } from "lucide-react";
  ...
  return (
    <div>
      <PageHeader
        ...
        actions={
          <Button onClick={() => toast("Faturë e re — demo lokale.")}>
            <Plus className="size-4" /> Faturë e re
          </Button>
        }
      />
      ...
      <div className="mb-4 flex flex-col gap-2 sm:flex-row">
        <div className="flex h-11 flex-1 items-center gap-2 rounded-xl border border-slate-200 bg-slate-100 px-3">
          ...
          <input
            ...
            placeholder="Kërko faturë, klient ose referencë..."
      ...
      <Card className="overflow-hidden">
        {rows.length === 0 ? (
          <EmptyState
            icon={Receipt}
            title="Asnjë faturë ende"
            description="Krijoni një faturë nga një ofertë e pranuar ose nga e para."
            action={
              <Button onClick={() => toast("Faturë e re — demo lokale.")}>
                <Plus className="size-4" /> Faturë e re
              </Button>
                ...
                {rows.map((inv) => (
                  ...
                  >
                    ...
                    <td className="px-5 py-4 text-right font-semibold text-slate-900">
                      {eur(invoiceTotal(inv), { symbolAfter: true })}
                    </td>
                  </tr>
                ))}
              </tbody>
...
function Tile({
  ...
  return (
    <Card className="p-5">
      <span className={`mb-3 inline-grid size-10 place-items-center rounded-xl ${tone}`}>
